...
def install_pystencils_git(session: nox.Session, revision="v1.x"):
    session.install(f"git+https://i10git.cs.fau.de/pycodegen/pystencils.git@{revision}")
...
def typecheck(session: nox.Session):
    ...
    install_pystencils_git(session)
...
def testsuite(session: nox.Session, platform: str):
    if platform == "cupy":
        install_cupy(session, "12.0", skip_if_no_cuda=True)
    install_pystencils_git(session)
