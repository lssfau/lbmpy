...
def add_fluctuations_to_collision_rule(
    collision_rule,
    temperature=None,
    amplitudes=(),
    *,
    rng: "RngBase | None" = None,
    ...
    if rng is None:
        rng = Philox(
            "fluctuation_rng", np.float32, TypedSymbol("seed", DynamicType.INDEX_TYPE)
        )
    ...
    for i, corr in enumerate(correction):
        collision_rule.main_assignments[i] = Assignment(
            collision_rule.main_assignments[i].lhs,
            collision_rule.main_assignments[i].rhs + corr,
        )


def fluctuation_amplitude_from_temperature(
    method, temperature, c_s_sq=sp.Symbol("c_s") ** 2
):
    """Produces amplitude equations according to (2.60) and (3.54) in Schiller08"""
    normalization_factors = sp.matrix_multiply_elementwise(
...
def fluctuation_correction(method, rng_generator, amplitudes=SymbolGen("phi")):
    ...
    random_matrix = sp.Matrix(
        [
            0 if m in conserved_moments else (next(rng_generator) - 0.5) * sp.sqrt(12)
